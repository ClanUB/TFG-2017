...
def getQual2process(id_ens, data):
    asigMap = map_asig()
    qual2process = data[id_ens] #Seleccion de la enseñanza
    ...
    qual2process.dropna(axis = 0, how='any',subset=['id_assig'], inplace=True) #Eliminar las filas cuyas id_assig no se reconocen
...
def loadData(id_ens):
    ...
    index_m15_2 = pivot_year2[pivot_year2.sum(axis=1)<15].index
    index_m15 = list(set(index_m15_1)|set(index_m15_2))
    
    index_del = list(set(index_nan)|set(index_m15))
    pivot_conc = pd.concat([pivot_year1, pivot_year2], axis = 1)
